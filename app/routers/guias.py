...
async def listar_guias(
    ...
    if fecha_inicio:
        query = query.filter(WHTransaction.TransactionDate >= parse_date_filter(fecha_inicio, is_end_date=False))
    if fecha_fin:
        query = query.filter(WHTransaction.TransactionDate <= parse_date_filter(fecha_fin, is_end_date=True))
