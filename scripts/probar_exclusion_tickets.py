#!/usr/bin/env python
"""
Script de prueba para validar que la exclusión de tickets (IDs que empiezan con 'T')
funciona correctamente tanto a nivel de consultas de base de datos como a nivel de API (HTTP).
"""
import os
import sys
...
def test_db_queries():
    ...
    try:
        ...
        total_tickets = db.query(func.count(ARDocument.Document)).filter(ARDocument.Document.like('T%')).scalar()
        total_permitidos = db.query(func.count(ARDocument.Document)).filter(~ARDocument.Document.like('T%')).scalar()
        
        print(f"Total de documentos en la tabla AR_Document: {total_real}")
        print(f"Total de tickets (inician con 'T'):          {total_tickets}")
        ...
        ticket_muestra = db.query(ARDocument.Document).filter(ARDocument.Document.like('T%')).first()
...
def test_api_endpoints(ticket_id):
    ...
    # Para saltar la autenticación de los endpoints de ventas/dashboard,
    # el script simula las peticiones. Dado que en producción require_authenticated y require_ventas_access
    # leen tokens JWT, usaremos un token ficticio o simularemos una llamada directa al router.
    # Para mayor simplicidad y certeza, podemos importar los routers directamente o burlar la dependencia.
    # Sin embargo, dado que TestClient llamará las funciones, podemos simplemente mockear la autenticación:
    from app.routers import auth
    # Mockear temporalmente las dependencias de autenticación para que retornen un usuario mock
    ...
    try:
        # A. Probar Listado de Ventas (/ventas/)
        print("Consultando listado de ventas a través de la API...")
        response = client.get("/ventas/?page=1&page_size=50")
        if response.status_code == 200:
            data = response.json()
            items = data.get("data", {}).get("items", [])
            print(f"  - Total items retornados por la API en la página 1: {len(items)}")
            
            tickets_encontrados = [item for item in items if item["Document"].startswith("T")]
            if tickets_encontrados:
                ...
                print("  ✅ CORRECTO: No se retornó ningún ticket en el listado de la API.")
        else:
        ...
        if ticket_id:
            ...
            response_ticket = client.get(f"/ventas/{ticket_id}")
